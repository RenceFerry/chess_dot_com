      ...
      { 
        playersArr.map((player: PlayersType, i: number) => (
          <Button onClick={() => viewProfile(player.id)} bgspan='fore/20' key={i} className='mx-auto flex flex-row h-18 md:h-20 rounded-full w-95/100 md:w-[80%] max-w-200 bg-back px-1 relative shrink-0 transform transition-all ease-in-out duration-300 hover:scale-103 hover:brightness-125 cursor-pointer'>
            ...
            {/** profile pic */}
            <Image src={player.image || 'https://khqlrecfncqrctilbjwx.supabase.co/storage/v1/object/public/avatar/no_profile%20(1).jpg'} alt='profile pic' className='h-16 md:h-18 md:w-18 w-16 rounded-full my-auto' width={90} height={90} loading='eager' unoptimized={false} />

            {/** player details */}
            <div className="flex-1 flex flex-row py-2 px-4 items-center justify-between ">

              {/** name */}
              <h1 className='text-md font-semibold md:text-xl truncate'>{player.name}</h1>
